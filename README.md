...
synthetic seeds — **296 seed files total now** (7 from VLAN/Modbus/DNS
...
**Roadmap status: complete, with two deliberate exceptions, plus four
protocols found through this project's own two pcap survey passes
...
see their own table entries above). This two-pass survey approach is
worth noting for its own sake: the first pass alone would never have
surfaced MACsec/HomePlug AV/Loopback, since none of the three involve
an IP protocol number or a TCP/UDP port at all — they're pure
EtherType-level traffic, checkable only by asking a genuinely
different question of the same data.

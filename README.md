...
| GTPv2-C | 3GPP TS 29.274 | `dpi_gtp_parser.c` | Message type, TEID (if present), sequence number, and **13 Information Element types**: IMSI, MSISDN (BCD-decoded, verified round-trip), APN (label-decoded), Cause, Recovery/Restart Counter, RAT Type, F-TEID (interface type + TEID + IPv4/IPv6 address), PDN Address Allocation (IPv4/IPv6/IPv4v6), Charging ID, EPS Bearer ID, AMBR, Serving Network (MCC/MNC), and Bearer QoS (**fully decoded** — PCI/PL/PVI bit fields and all four rate fields, following authoritative spec research; see the breadth-extensions section below for the full story). F-TEID, PAA, and Charging ID verified against constructed test vectors. IMSI/MSISDN are subscriber PII — flagged with a privacy note, same discipline as RADIUS's `User-Password` handling. |
...
| BitTorrent Mainline DHT | BEP 3 (bencoding) / BEP 5 (DHT) | `dpi_bt_dht_parser.c` | UDP port 6881, publicly documented (unlike several "reverse-engineered from scratch" protocols in this project). Verified against **126,321 real payloads** in a genuine capture — 100% decoded with zero parse failures, the largest, cleanest real-traffic sample in this entire project. Every real message was a genuine `get_peers` query (real 20-byte node ID and info_hash, hand-decoded byte-for-byte before writing any C), consistent with the capture's name ("background": a client idling on one torrent, periodically re-querying the DHT). Extracts message type, transaction ID, query method, and — from the arguments sub-dictionary — node ID, info_hash, and target (the latter two shared by `ping`/`find_node`/`announce_peer` per BEP 5 but not real-traffic-verified for this file specifically). Response/error sub-dictionaries (compact-encoded node/peer lists in particular) are named but not decoded further — a genuinely more involved packed binary encoding needing its own verification. Bounded bencode parsing (depth and dictionary-size limits) matches this project's discipline for every other variable-length-field format (BER, HTTP/2 framing). |
| SCTP | RFC 4960 | `dpi_sctp_parser.c` | **A transport-layer protocol, a peer to TCP/UDP, not an application protocol riding on either** — IP protocol 132, needing its own capture-path branch the same way ESP/AH/L2TPv3 do. This was this project's own roadmap's explicitly-recommended "build first" item, since nothing riding on top of SCTP (M2UA, M3UA, and whatever SS7-over-IP signaling those carry) is reachable without it existing. Verified against 500 real packets across 2 genuine captures — common header, chunk framing (including real bundled/multi-chunk packets), and all 4 chunk types that appeared in real traffic (HEARTBEAT, HEARTBEAT_ACK, SACK, DATA) hand-decoded byte-for-byte, 500/500 (100%) detected correctly. A real SACK's cumulative-TSN-ack field was confirmed to reference the exact same TSN value seen in a real DATA chunk. Every real DATA chunk's Payload Protocol Identifier resolved to a real, IANA-registered SIGTRAN value (M3UA in one capture, M2UA in the other) — recursively dispatched via PPID, the same "extract what's actually there" pattern GRE/MPLS/L2TPv3/802.11 already use for their own inner payloads. INIT/COOKIE/ABORT/SHUTDOWN/ERROR chunk types are named only — none appeared in the real traffic checked, which showed only association-maintenance and steady-state signaling, not connection setup/teardown. |
| M3UA | RFC 4666 | `dpi_m3ua_parser.c` | **Reached via SCTP's PPID-keyed recursion, not its own transport-layer port** — the natural next step once SCTP existed to carry it. Verified against all 7 real M3UA messages found in a genuine SS7-over-IP call-signaling capture, every one hand-decoded byte-for-byte before writing any C: 7/7 share the exact same shape (Message Class 1/Transfer, Message Type 1/DATA, and precisely 3 parameters in the same order — Network Appearance, Routing Context, Protocol Data). Routing Context was identically `0x00000015` across all 7 real messages (the same signaling relationship throughout this one real call); Network Appearance alternated between two real values. A real SACK-chunk field from `dpi_sctp_parser.c`'s own verification and this file's Protocol Data extraction were cross-checked against each other for internal consistency. Protocol Data (the actual nested MTP3-User payload — SCCP or ISUP) is extracted as a raw hex blob, not decoded further — a genuinely deeper layer this project doesn't have confirmed, verified knowledge of the exact national/international MTP3 label format or SCCP/ISUP message layout to decode with the same confidence as everything else here, stated as a deliberate scope boundary rather than guessed at. Message classes other than DATA (Management, SS7 Signalling Network Management, ASP State/Traffic Maintenance, Routing Key Management) are named only per RFC 4666 — none appeared in real traffic checked. |
| AMQP 0-9-1 | AMQP 0-9-1 spec | `dpi_amqp_parser.c` | TCP port 5672, publicly and officially specified (unlike several "reverse-engineered from scratch" protocols in this project). Verified against 42 real, complete frames across 3 genuine captures — but only after finding and fixing a real methodology bug in this project's own verification process: an initial per-packet-only check found 214 of 254 real payloads "malformed," which turned out to be nothing of the kind — AMQP frames routinely span multiple TCP segments, and checking raw individual segments in isolation meant landing mid-frame and misreading real message data as a nonsensical frame header. Re-verified against each real TCP flow's segments concatenated in capture order (the same reassembled form this project's TCP flow reassembly actually delivers to every TCP-based dissector) — malformed count dropped to exactly zero, 42/42 real frames parsed cleanly. Those 42 frames span 20 distinct class/method combinations, and turned out to be real Celery (the Python task-queue framework) traffic — every real `Basic.Publish`/`Basic.Deliver` frame checked used exchange `"celeryev"` and routing key `"worker.heartbeat"` (Celery's real event-exchange and worker-heartbeat pattern), and the one real `Queue.Declare` showed a real anonymous/exclusive/auto-delete temporary queue. Argument extraction is deliberately narrow: only fields using AMQP's simplest encoding (`shortstr`) that are also real-traffic-verified — `Basic.Publish`/`Basic.Deliver`'s exchange and routing key, `Queue.Declare`'s queue name. AMQP's nested `table` encoding (used by `Connection.StartOk`'s client-properties among others) is not decoded — a real, more involved format this project doesn't have a verified byte-exact layout for, matching the same discipline as M3UA's un-decoded Protocol Data parameter. `HEADER`/`BODY` frame types are named and their basic framing validated, but their own payloads (content properties, and the raw message body — arbitrary application data) aren't decoded further. |
| M2UA | RFC 3331 | `dpi_m2ua_parser.c` | Reached via SCTP's PPID-keyed recursion (PPID 2), the identical pattern `dpi_m3ua_parser.c` established for PPID 3. Verified against the one real M2UA message available — Message Class 6 (MAUP), Message Type 1 (Data), declared length matching actual length exactly. Only one real sample exists, reflected honestly via a lower 0.85 detection confidence rather than M3UA's 0.9. RFC 3331 is a stable, long-published standard, cross-checked across 6 independent sources. MTP2 User Peer-to-Peer Message Data extracted as raw hex, not decoded further — same scope boundary as M3UA's Protocol Data. |
| PIM | RFC 7761 / RFC 6166 | `dpi_pim_parser.c` | IP protocol 103. Verified against one real Hello message — Version 2, Type 0, Holdtime and LAN Prune Delay options both decoding cleanly. All 11 officially-registered message types named per RFC 6166's stable IANA registry; only Hello's option format is decoded, since it's the only one with real traffic to verify against. |
| AppleTalk | DDP, EtherTalk Phase 2 | `dpi_appletalk_parser.c` | Legacy, low-relevance, included for roadmap completeness. SNAP-encapsulated (Apple's OUI 08:00:07, embedded EtherType 0x809B). A real bug caught while wiring this in: an earlier dispatch check routed every LLC-framed frame to STP alone, which would have made AppleTalk permanently unreachable — fixed before it was ever real-traffic-verified. Verified against 2 real, identical frames decoding to an internally coherent RTMP broadcast (declared DDP length matched actual bytes exactly, socket numbers matched RTMP's well-known value). DDP Type named only for the one confirmed value. |
| PPPoE | RFC 2516 | `dpi_pppoe_parser.c` | Real EtherTypes 0x8863/0x8864, reached via `dispatch_by_ethertype()`. Not real-traffic-verified — built with real confidence regardless, since RFC 2516 is a stable 1999 standard cross-checked identical across the RFC Editor, IETF, and independent vendor docs (Huawei, Nokia). Structurally verified against the RFC's own worked PADO example byte-for-byte. CODE values, TAG format (Service-Name/AC-Name/Host-Uniq), and the Session-stage PPP Protocol field are all decoded; the actual PPP payload itself is not. |
| CDP | Cisco Discovery Protocol | `dpi_cdp_parser.c` | SNAP-encapsulated (Cisco's OUI 00:00:0c, embedded PID 0x2000). Not real-traffic-verified in this project's own captures — built with real confidence from the Wireshark wiki's complete, real, field-by-field decoded worked example, independently corroborated by Cisco's own IOS documentation. Header fields plus Device-ID/Addresses(count only)/Platform/Native-VLAN/Duplex TLVs decoded; the Address TLV's own nested NLPID-tagged address entries are not. |
| EAPOL | IEEE 802.1X | `dpi_eapol_parser.c` | Real EtherType 0x888E. Not real-traffic-verified — built with confidence from IEEE 802.1X's stable, consistently-documented 4-byte header format (cross-checked across a Wireshark-based real-packet walkthrough and HPE's own documentation). All 5 packet types named; for EAP-Packet specifically, the inner RFC 3748 EAP header (Code/Identifier/Length/Type) is decoded too. EAPOL-Key's actual WPA/WPA2 handshake material is deliberately not decoded — security-sensitive key-exchange content this project won't guess the layout of. |
| LACP | IEEE 802.3ad / 802.1AX | `dpi_lacp_parser.c` | Real EtherType 0x8809. **A real bug caught and fixed during its own verification**: an initial TLV-length constant was off by the 2-byte TLV header itself, which would have silently rejected every real Actor/Partner Information TLV. Verified against real tcpdump-captured bytes from two independent public sources — a real Actor TLV's System Priority, MAC, Key, and all 5 State flag bits (Activity/Aggregation/Synchronization/Collecting/Distributing, Timeout correctly absent) matched exactly after the fix. Collector Information/Terminator TLVs recognized by type only. |
| RTSP | RFC 2326 / RFC 7826 | `dpi_rtsp_parser.c` | TCP port 554. Modeled directly on this project's existing SIP/HTTP1 line-and-header parsing pattern, since RTSP's own RFCs describe it as HTTP-like in framing. Not real-traffic-verified — checked structurally against a standard DESCRIBE request. Request/status line, plus CSeq/Session/Transport/Content-Type headers decoded; the SDP body a DESCRIBE response typically carries is not (SDP is its own separate format this project doesn't have a dissector for). |
| MySQL | MySQL client/server protocol | `dpi_mysql_parser.c` | TCP port 3306. Focuses on the Initial Handshake packet (protocol version 10) — the single highest-value packet for passive fingerprinting. Verified against a real, complete documented handshake packet from the mysql-proxy project — packet length, sequence ID, protocol version, server version string ("5.5.2-m2"), connection ID, and capability flags all matched exactly. Later connection/command-phase packets (HandshakeResponse, queries, result sets) are not decoded — a real, substantial state machine this project doesn't attempt without traffic to verify the decision points against. |
| PostgreSQL | PostgreSQL wire protocol | `dpi_postgresql_parser.c` | TCP port 5432. Focuses on the StartupMessage — real cleartext protocol version, database, and user before any authentication. Verified against PostgreSQL's own official documentation (identical across 8 major versions spanning 15 years) plus a real worked example from the PostgreSQL mailing list archives, which decoded correctly into its two real parameters. SSLRequest/CancelRequest/GSSAPIRequest special codes are named but not decoded further. Post-startup messages (Query, RowDescription, DataRow, etc.) are not decoded. |
...
| MACsec | IEEE 802.1AE | `dpi_macsec_parser.c` | **Found via a second, EtherType-focused pcap survey pass** (the first pass checked IP protocols and TCP/UDP ports only) — real EtherType 0x88E5, 16 real packets in `ultimate.pcapng`. Verified with unusually high confidence for a protocol with no dedicated capture of its own: the SecTAG byte layout was cross-checked against IEEE 802.1AE-2006's own standard text, a Linux kernel netdev patch, and the tcpdump project's own `print-macsec.c` source code. Applied to the real capture, the decoded Secure Channel Identifier's MAC-address component (`00:0c:29:55:9b:4b`) is a genuine, independently-recognizable VMware-registered OUI — real confirmation the byte offsets are correct, not just a plausible-looking parse. The SecTAG (association number, packet number, secure channel identity) is fully decoded; the actual protected payload is, by MACsec's whole design, encrypted or integrity-protected — the same reason this project doesn't decode inside TLS/QUIC's encrypted records either. |
| HomePlug AV | Powerline networking (Intellon/Qualcomm Atheros) | `dpi_homeplugav_parser.c` | Found via the same EtherType survey — real EtherType 0x88E1, 28 real packets in `ultimate.pcapng`. The 6-byte management-frame header (version/type/OUI) is confirmed across the `faifa` open-source HomePlug tool's own manpage and Wireshark's EtherType registry; applied to the real capture, the decoded OUI (`00:b0:52`) is Intellon Corporation's own genuine, historically-registered OUI (Intellon being HomePlug AV's original developer) — the same category of independent confirmation MACsec's VMware OUI provided. The variable-format, vendor-specific payload is not decoded — by the protocol's own real-world tooling documentation, it isn't a single fixed structure to begin with. |
| Ethernet Loopback | Vendor keepalive/diagnostic convention (no formal spec) | `dpi_ethloopback_parser.c` | **Deliberately detection-only, stated honestly** — real EtherType 0x9000, 127 real packets in `ultimate.pcapng` (payload all zeros, consistent with a keepalive carrying no meaningful content). Different in kind from DECnet/Banyan VINES's own detection-only scope: Wireshark's own wiki states this protocol "doesn't appear in any IEEE 802 specification" at all — there's genuinely no payload structure to decode, not just one this project hasn't found yet. |
| OpenVPN | OpenVPN protocol | `dpi_openvpn_parser.c` | Found via a fifth pcap survey pass, this time re-checking the full port list (not just the top 20 by count) — 1,778 real packets on UDP port 1194 in `set3.pcap`. The opcode/key_id byte structure was cross-checked across 4 independent sources (OpenVPN's own docs, the Wireshark project's own dissector source code, a third-party security assessment, a Zeek analyzer write-up), all agreeing. Every real packet decoded to opcode 6 (P_DATA_V1 — actual encrypted tunnel data), consistent with steady-state VPN traffic. Opcode and key_id are decoded; the payload itself is, by design, encrypted — the same reason this project doesn't decode inside TLS/QUIC/MACsec's encrypted portions either. |
| XMPP | RFC 6120 | `dpi_xmpp_parser.c` | Found via the same fifth pass — 611 real packets on TCP port 5222 in `nitroba.pcap`. About as unambiguous as real-traffic verification gets: the payload contained a literal `<?xml version='1.0' ?>` declaration followed by a real `<stream:stream>` tag addressed to a genuine service (`to='gmail.com'`), and a real server response (`from="gmail.com"`). The `to`/`from` stream attributes are extracted; this project deliberately does not implement a general XML parser or walk individual stanzas (`<iq/>`/`<message/>`/`<presence/>`) — real, substantial additional work beyond a bounded attribute extraction, not attempted without much broader real traffic to verify stanza-level structure against. |
...
| GTPv2-C: only 4 IE types decoded | `dpi_gtp_parser.c` | **Closed**: 13 IE types now (IMSI, MSISDN, APN, Cause, Recovery, RAT Type, F-TEID, PDN Address Allocation, Charging ID, EPS Bearer ID, AMBR, Serving Network, Bearer QoS) — F-TEID, PAA, and Charging ID verified against constructed test vectors; Bearer QoS is now fully decoded (PCI/PL/PVI bit fields + all four rate fields), following authoritative spec research rather than left partial. A handful of less common IEs (e.g. Protocol Configuration Options, Bearer Context) remain unwalked — their exact type numbers or nested structure aren't things this project has verified with confidence. |
...
| `dpi_dns_parser.c` | DNS (RFC 1035) dissector: header + question section, with bounds-checked, cycle-safe name decompression — verified against an adversarial cyclic-pointer test case, since this is a classic real-world DNS parser bug source. Also handles LLMNR (RFC 4795, UDP port 5355) — folded into this same dissector rather than a separate file, since its wire format is confirmed identical (see the "Fully parsed" table entry). Answer/authority/additional records not yet walked. | `dpi_dissector_registry.c` |
...
| `dpi_http1_parser.c` | HTTP/1.1 request/status line + Host/User-Agent/Content-Type header extraction, plus a bounded body preview (200 bytes) and true total body length — added on direct request. | `dpi_dissector_registry.c` |
...
| `dpi_rfc_parser.c` | No | **Yes — real bugs found and fixed** | IPv4 fragmentation reassembly had a severe, previously undisclosed bug (completion check could never succeed for realistically-sized datagrams) — found and fixed against `ipfragments.pcap`'s 8 real fragment sets, re-verified 8/8 across multiple random re-orderings. A second, already-disclosed gap (mid-hole fragment splitting) was also closed after confirming it mattered under real out-of-order arrival. A fourth pcap survey pass (checking real TCP option `kind` values against what was parsed) found TCP SACK blocks (RFC 2018, option kind 5) were already being parsed into `struct tcp_parsed_options` — 16,573 real occurrences across the pcap set, all 4 possible block counts (1–4) genuinely present — but the parsed data was never actually surfaced anywhere in the JSON output, a real wiring gap distinct from a parsing gap. Fixed: now exposed as `sack_blocks_seen` in every TCP flow's `reassembly` object. |
...
| `dpi_http1_parser.c` | No | No | No chunked transfer-encoding reassembly; body preview extraction verified against a real 302 redirect response with a genuine HTML body (real bytes, not fabricated) — only attempted when a genuine blank-line header terminator is actually found, not guessed at from an unreliable position |
...
| `dpi_icmp_parser.c` | No | No | Embedded original-packet dissection implemented for both ICMPv4 (ports only) and ICMPv6 (full TCP/UDP header) — verified against constructed test packets. ICMPv4 type coverage extended after a third pcap survey pass checked every real (type, code) pair seen against what was named: Router Advertisement/Solicitation and Information/Address Mask Request all real-traffic-verified, including a real Router Advertisement's sensible 1800-second lifetime and a real Address Mask Request's correctly-all-zero mask. The same survey caught a real methodology bug in itself: two (type, code) pairs that initially looked like unknown types turned out to be non-first IP fragments (raw ping-padding continuation bytes misread as a fresh ICMP header) — excluded once checked, not shipped as a false finding. |
...
| `dpi_dns_parser.c` | No | No, for DNS's own name decoding | Query name decoding (cycle-safe compression pointers) verified against a constructed adversarial cyclic-pointer case, not real traffic directly — though `dpi_mdns_parser.c` reuses this same logic and IS real-traffic-verified (339 real packets). LLMNR support added to this same file IS real-traffic-verified: a real query for "wpad" (QTYPE=ANY, QCLASS=IN) found in a real capture, port 5355, byte-for-byte identical to DNS's own header/question layout. Answer-record type coverage extended after the same third pcap survey pass that extended ICMP — TXT/PTR/SRV all real-traffic-verified (a real SRV record decoded to a plausible real target, "pm-members.mac.com", port 5354); SOA is partially covered (MNAME and Serial only — RNAME and the remaining 4 fixed fields are walked to stay correctly positioned but not themselves reported, a deliberately narrower scope than the other three given SOA's added complexity). |
...
| `dpi_bt_dht_parser.c` | No | **Yes, extensively** | 126,321 real UDP payloads, 100% decoded with zero parse failures — the largest, cleanest real-traffic sample in this project. Every real message was a `get_peers` query, hand-decoded byte-for-byte before writing any C; `ping`/`find_node`/`announce_peer` share the identical arguments-dictionary shape per BEP 5 but aren't real-traffic-verified for this file. Response/error sub-dictionaries (`nodes`/`values` compact encoding) are named but not decoded. |
| `dpi_sctp_parser.c` | No | **Yes** | 500 real packets across 2 genuine captures, 100% detected correctly — common header, chunk framing (including real bundled multi-chunk packets), and all 4 chunk types that appeared in real traffic (HEARTBEAT/HEARTBEAT_ACK/SACK/DATA) hand-decoded byte-for-byte. Every real DATA chunk's PPID resolved to a real IANA-registered SIGTRAN value. INIT/COOKIE/ABORT/SHUTDOWN/ERROR chunk types are named only — none appeared in real traffic checked. |
| `dpi_m3ua_parser.c` | No | **Yes** | All 7 real M3UA messages found in a genuine SS7-over-IP call-signaling capture, 7/7 hand-decoded byte-for-byte, all sharing the identical Transfer/DATA shape with the same 3-parameter structure. Protocol Data (the nested MTP3-User payload) deliberately extracted as raw hex, not decoded further — stated as a real scope boundary, not a gap. |
| `dpi_amqp_parser.c` | No | **Yes, and a real verification-methodology bug found & fixed** | 42 real, complete frames across 3 genuine captures, 100% clean after fixing this project's own naive per-packet-only check (which misread mid-frame TCP-segmented data as malformed — see the Fully Parsed table entry for the full story). Real Celery task-queue traffic — every real Basic.Publish/Basic.Deliver used exchange "celeryev" and routing key "worker.heartbeat". AMQP's nested `table` encoding not decoded — no verified byte-exact layout available. |
| `dpi_stp_parser.c` | No | **Yes** | 7 real, identical BPDU frames from a genuine capture, every field decoding to a real, sensible RSTP configuration — textbook-default timers, a converged-topology flag combination, a root bridge genuinely distinct from the local bridge ID. A real precision bug caught and fixed during verification itself: an initial extraction included real Ethernet padding as if it were BPDU content, since the frame was shorter than Ethernet's 60-byte minimum; fixed by bounding parsing to the 802.3-declared length instead. MSTP (version 3) extensions named but not decoded — no real MSTP traffic available. |
| `dpi_m2ua_parser.c` | No | **Yes, but thin** | Only one real M2UA message available, stated honestly (lower detection confidence than M3UA's, not equalized). RFC 3331's message table itself is on firm ground (a stable, long-published standard, cross-checked across 6 sources), even though real-traffic coverage of it is thin. |
| `dpi_pim_parser.c` | No | **Yes, but thin** | One real Hello message. RFC 6166's message-type registry is stable and official; only Hello's own option format is real-traffic-verified. |
| `dpi_appletalk_parser.c` | No | **Yes** | 2 real, identical frames, internally self-consistent (declared DDP length matched actual bytes exactly). A real dispatch-routing bug caught and fixed while wiring this in — see the Fully Parsed table entry. |
...
gcc -O3 -march=native $(pkg-config --cflags libdpdk) \
    -o dpi_dpdk_worker dpi_dpdk_worker.c $(pkg-config --libs libdpdk) -lssl -lcrypto -lm
...
This one was explicitly listed as a known, disclosed limitation
(`http2_continuation_split_mid_frame_not_reassembled`) rather than a
hidden bug — the existing cross-TCP-boundary CONTINUATION reassembly
only handled a split landing cleanly AT a frame boundary (not enough
buffer for even the next frame's header), not a split in the MIDDLE of
a CONTINUATION frame's own payload (header fully present, payload
isn't). Closed it properly rather than leaving the disclosure in
place indefinitely.

**The fix**: `dpi_hpack_connection_state.c` gained a second piece of
per-connection state (`partial_cont_*` fields) alongside the existing
`pending_block`, specifically for the ONE in-flight CONTINUATION
frame whose payload is still incomplete — distinct from
`pending_block`, which only ever holds complete frames' worth of
already-extracted header-block bytes. `dpi_http2_parser.c` gained
`http2_resume_partial_continuation()`, called at the start of the next
delivery to complete that one frame from its beginning, before falling
through to the normal frame-by-frame scan for whatever follows.

**Verified against constructed multi-delivery scenarios** covering all
three shapes that matter: a clean two-way split, a three-way split
needing two separate resume calls to finish, and a split where the
completing frame lacks END_HEADERS and a further CONTINUATION frame
follows immediately in the same delivery (confirmed the scan correctly
picks up scanning fresh frames from exactly the right position
afterward). All three verified byte-for-byte correct against the
original, unsplit payload. Stated honestly: this is logic-verified via
construction, not real-traffic-verified — no capture available to
this project happened to include a TCP delivery boundary landing
inside a CONTINUATION frame's payload specifically, a genuinely rare
occurrence even in real HTTP/2 traffic.

**Added a dedicated new fuzz harness**, `fuzz_http2_continuation_
reassembly.c`, specifically because the existing HTTP/2 harness only
exercises the connectionless, single-call `http2_dissect()` entry
point and provides zero coverage of the stateful
`http2_dissect_with_flow_state()` path this fix lives in — same
reasoning as the dedicated harnesses already added for IPv4
fragmentation and TCP reassembly. 4 seeds cover the two-way split,
three-way split, split-then-more-frames, and a frame-boundary-split
baseline matching the case that already worked before this fix.

The one remaining, still-disclosed gap: a split in the middle of a
CONTINUATION frame's own 9-byte header (as opposed to its payload) is
not tracked — RFC 9113's fixed, tiny header essentially never gets
split by itself in practice, and this project's bounded-state
...
without the spec text in hand.

**Bearer QoS's PCI/PL/PVI bit fields and four rate fields, previously
left deliberately partial pending spec access, are now fully decoded**
— the missing piece was simply going and finding the actual spec text
rather than continuing to defer indefinitely. Checked the authoritative
3GPP TS 29.274 §8.15 text directly (a 3GPP CT4 change-request document
showing the exact octet/bit table), cross-confirmed independently
against a Cisco SGSN admin guide that separately cites the same
clause — two sources agreeing rather than one uncertain memory. Octet
5's bit layout (bit 7 = PCI, bits 6-3 = PL, bit 1 = PVI, using 3GPP's
own bit-numbering convention where bit 8 is an octet's MSB) and the
four 5-byte rate fields' encoding (confirmed to be a plain 40-bit
big-endian integer in kbps, not a packed or exponential format, per
the spec's own "encoded as kilobits per second... in binary value"
wording) were both verified against multiple constructed test vectors
before being written into C — including confirming the bit-extraction
formulas reproduce PCI/PL/PVI correctly across three different bit
patterns, and the rate decoding reproduces exact kbps values including
a 1 Gbps case. A new synthetic seed exercising the full IE was added
to the existing `fuzz_seeds/gtp/` corpus.
...
| BitTorrent Mainline DHT (new protocol) | **126,321 payloads** | **100%, zero parse failures — largest, cleanest real-traffic sample in this project** |
| SCTP (new protocol, roadmap item) | 500 packets, 2 captures | 100% detected — real DATA-chunk PPIDs resolved to real SIGTRAN values (M3UA, M2UA) |
| M3UA (new protocol, roadmap item) | 7 real messages | 7/7 (100%) — all sharing an identical, verified structure |
| AMQP 0-9-1 (new protocol, roadmap item) | 42 real frames, 3 captures | 100% — but only after finding & fixing a real bug in this project's own verification methodology first |
| STP/RSTP (new protocol, roadmap item) | 7 real, identical BPDU frames | 100% — a real padding-vs-content precision bug caught during verification, not after |
| M2UA (new protocol, roadmap item) | 1 real message (thin) | 100% of what's available — confidence scored lower accordingly |
| PIM (new protocol, roadmap item) | 1 real Hello message (thin) | 100% of what's available |
| AppleTalk (new protocol, roadmap item) | 2 real, identical frames | 100% — a real dispatch-routing bug caught and fixed during wiring, not after |
...
synthetic seeds — **299 seed files total now** (7 from VLAN/Modbus/DNS
...
exactly. MACsec, HomePlug AV, and Ethernet Loopback — found afterward,
via a second EtherType-focused pcap survey pass — were wired into
both capture paths from the start, so there's no equivalent gap for
those three to close.
...
|---|---|---|
| **SCTP** (IP proto 132) | 500 real packets across `bssmap_bsc_invoke_trace.pcap` + `raaw-call.pcap` — common header, HEARTBEAT/HEARTBEAT_ACK/SACK/DATA chunks all decoded | **Done** — see `dpi_sctp_parser.c` and the "Fully parsed" table above; 100% detection across all 500 real packets |
| **M3UA** (SCTP PPID 3, port 2905) | 500 real packets in `raaw-call.pcap` — a real DATA message decoded end to end, including Network Appearance/Routing Context/Protocol Data parameters | **Done** — see `dpi_m3ua_parser.c`; 7/7 real messages found in this capture decoded correctly |
| **M2UA** (SCTP PPID 2, port 2904) | 1 real packet in `bssmap_bsc_invoke_trace.pcap` — a real MAUP-class Data message decoded | **Done** — see `dpi_m2ua_parser.c`; the one available real message decodes correctly, confidence scored honestly lower than M3UA's given the thinner sample |
| **AMQP 0-9-1** | 42 real, complete frames across 3 genuine captures — real Celery task-queue traffic, 20 distinct class/method combinations | **Done** — see `dpi_amqp_parser.c`; 100% after finding and fixing a real bug in this project's own verification methodology (per-packet-only checking misread TCP-segmented frames as malformed) |
| **STP** (802.1D, LLC DSAP/SSAP 0x42) | 7 real, identical BPDU frames in `Paging_Request.pcap` — every field decoded to a real, sensible RSTP configuration | **Done** — see `dpi_stp_parser.c`; 7/7 real frames decoded correctly, including a real padding-vs-content precision bug caught during verification |
| **LLMNR** (UDP 5355) | Real "wpad" query in `Paging_Request.pcap`, wire-format-identical to DNS | **Done** — folded into `dpi_dns_parser.c`, the same way RARP was folded into ARP; the real "wpad" query decodes correctly with a 0.85 detection confidence, distinguished from ordinary DNS via a `dns_is_llmnr` field |
| **PIM** (IP proto 103) | Real Hello message in `Paging_Request.pcap` | **Done** — see `dpi_pim_parser.c`; the real Hello message's Holdtime and LAN Prune Delay options both decode correctly |
...
**Roadmap status: complete, with two deliberate exceptions, plus six
protocols found through this project's own five pcap survey passes
rather than a predetermined list.** SCTP, M3UA, M2UA, AMQP, the DHCP
option breadth extension, LLMNR, STP, PIM, AppleTalk, PPPoE, CDP,
EAPOL, LACP, RTSP, MySQL, PostgreSQL, TDS, NTP extension fields,
DECnet, Banyan VINES, Apple's `serialnumberd` (found via a first
survey pass checking every capture for a TCP/UDP port with real
traffic and no matching dissector), and — found via a **second**
survey pass, this time checking EtherTypes specifically rather than
IP protocols/ports — MACsec, HomePlug AV, and Ethernet Loopback are
all done (DECnet, Banyan VINES, and Ethernet Loopback deliberately
detection-only, stated honestly, each for its own specific reason —
see their own table entries above). A **third** pass checked field-
level type/code coverage on already-built dissectors (ICMP and DNS)
rather than hunting new protocols — see those two dissectors' own
honest-status entries below. A **fourth** pass audited data flow
rather than parsing coverage, and found TCP SACK blocks (RFC 2018)
were already being parsed but never actually surfaced in the JSON
output — a wiring gap, not a parsing one; also see `dpi_rfc_parser.c`'s
own entry below. A **fifth** pass went back to port-level checking,
this time against the *full* port list rather than only the highest-
count entries, and found OpenVPN and XMPP — 1,778 and 611 real
packets respectively, both cross-verified against multiple
independent sources (see their own table entries above). This multi-
pass survey approach is worth noting for its own sake: the first pass
alone would never have surfaced MACsec/HomePlug AV/Loopback, since
none of the three involve an IP protocol number or a TCP/UDP port at
all — they're pure EtherType-level traffic, checkable only by asking
a genuinely different question of the same data. Two candidates
raised by these passes were checked carefully and correctly excluded
rather than built: a TCP port 37 "TIME protocol" candidate turned out
to be SYN-only scan traffic (every packet had `flags=0x02`, meaning
what looked like payload was almost certainly misread TCP options,
not real application data), and two ICMP (type, code) pairs that
initially looked like unknown types turned out to be non-first IP
fragments — both false positives caught before being reported as
real findings.

The flow-record correlation envelope
...
dedicated section above, now covering all 12 link-layer/discovery
protocols (the original 9 plus MACsec, HomePlug AV, and Ethernet
Loopback, wired in alongside them). The Cisco SNAP protocol and IrDA
remain unbuilt. Cisco SNAP — not from lack of time, but because this
project doesn't have confident, verified knowledge of its exact
sub-protocol format, and won't guess at wire-level structure the way
it has consistently declined to elsewhere (DNP3's field layout, IEEE
...
against something concrete before being trusted). *(Since updated:
Bearer QoS is now fully decoded — the source spec text was tracked
down rather than deferred on indefinitely; see the breadth-extensions

...
synthetic seeds — **274 seed files total now** (7 from VLAN/Modbus/DNS
...
| **M2UA** (SCTP PPID 2, port 2904) | 1 real packet in `bssmap_bsc_invoke_trace.pcap` — a real MAUP-class Data message decoded | Medium — SCTP prerequisite now satisfied; only one sample packet available, thin for full verification |
...
**Recommended build order, updated now that SCTP, M3UA, AMQP, the
DHCP option breadth extension, LLMNR, and STP are all done**: only
M2UA (thin real-traffic verification — one sample packet), PIM, and
AppleTalk remain, all lower priority than everything completed so
far — genuinely a matter of preference at this point rather than a
